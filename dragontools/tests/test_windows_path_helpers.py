...
def test_unc_path_is_normalized_without_host_reinterpretation() -> None:
    path = r"\\media-server\video\Serien\TV\American Dad! (2005)"

    assert normalize_user_path(path) == path
    assert user_path_name(path) == "American Dad! (2005)"
    assert user_path_parent(path) == r"\\media-server\video\Serien\TV"


def test_forward_slash_unc_and_backslash_unc_compare_equal() -> None:
    forward = "//media-server/video/Serien/Anime"
    backslash = r"\\media-server\video\Serien\Anime"

    assert normalize_user_path(forward) == r"\\media-server\video\Serien\Anime"
...
def test_path_child_check_is_case_insensitive_for_windows_paths() -> None:
    base = r"\\media-server\video\Serien\TV"
    child = r"//media-server/video/serien/tv/American Dad! (2005)"
    sibling = r"\\media-server\video\Serien\TV_alt\American Dad! (2005)"
