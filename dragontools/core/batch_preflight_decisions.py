...
def _pipeline_reason(preview: dict[str, Any]) -> str:
    ...
    labels = {
        "dv": f"Pipeline: Dolby Vision wird erhalten; deshalb Sonderpfad DV -> .{container}.",
        "av1_dv": f"Pipeline: AV1 Dolby Vision Profile 10 (Beta) -> .{container}.",
        "hdrplus": f"Pipeline: HDR10+ wird erhalten; deshalb Sonderpfad HDR10+ -> .{container}.",
        "av1_hdrplus": f"Pipeline: AV1 HDR10+ (Beta) -> .{container}.",
    }
    return labels.get(
        pipeline,
...
def _append_encoder_override_reason(reasons: list[str], overrides: dict[str, Any]) -> None:
    manual_encoder = overrides.get("encoder_override")
    if not isinstance(manual_encoder, dict):
        return
...
def _append_audio_reasons(reasons: list[str], preview: dict[str, Any]) -> None:
...
def _append_subtitle_reasons(reasons: list[str], preview: dict[str, Any]) -> None:
    subs = dict(preview.get("subtitles") or {})
    if int(subs.get("source_count") or 0) <= 0:
    ...
    if burn:
        ...
        blocked_reasons = {
            "ambiguous": "Untertitelregel: Forced-Burn-In bleibt offen, weil mehrere gleich gute Kandidaten existieren.",
            "auto_burn_disabled": "Untertitelregel: automatischer Forced-Burn-In ist deaktiviert.",
            "no_audio_for_burn_language": "Untertitelregel: kein Burn-in, weil keine passende Audio-Sprache vorhanden ist.",
        }
        reasons.append(blocked_reasons.get(blocked, "Untertitelregel: kein Burn-in-Kandidat gewählt."))
...
def _append_file_override_reasons(reasons: list[str], overrides: dict[str, Any]) -> None:
...
def _append_output_reasons(
    reasons: list[str],
    preview: dict[str, Any],
    fs_info: dict[str, Any] | None,
) -> None:
    ...
    final_path = _text((fs_info or {}).get("final_output_path") or (fs_info or {}).get("output_path"), "")
    if final_path:
...
def _decision_reasons(preview: dict[str, Any], fs_info: dict[str, Any] | None = None) -> list[str]:
    reasons = [_pipeline_reason(preview)]
    overrides = dict(preview.get("overrides") or {})
    _append_encoder_override_reason(reasons, overrides)
    _append_hdr_reasons(reasons, preview)
    _append_audio_reasons(reasons, preview)
    _append_subtitle_reasons(reasons, preview)
    _append_file_override_reasons(reasons, overrides)
    _append_output_reasons(reasons, preview, fs_info)
    return reasons
